...
import { FiCheckCircle, FiRefreshCw, FiShield, FiSliders, FiTruck } from "react-icons/fi";
  ...
  return (
    <div className="mx-auto">
      <div className="rounded-[30px] p-5 text-[#1A1A1A] shadow-side-bar sm:p-8">
        <p className="text-sm font-inter-r text-[#8798ad]">Manage global shipping APIs.</p>

        <div className="mt-6 grid grid-cols-3 gap-1 rounded-lg border border-white-color/10 bg-white-color/[5%] p-1">
          {partnerButtons.map(([id, item]) => (
            <button
              key={id}
              type="button"
              onClick={() => setActivePartner(id)}
              className={`h-10 rounded-md px-2 text-[11px] font-inter-b uppercase tracking-[0.08em] transition ${
                activePartner === id
                  ? "bg-yellow-color text-white shadow-sm"
        ...
        <div className="mt-9 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-inter-b text-white">{partner.name}</h2>
          ...
          <button
            type="button"
            onClick={() => notify(`${partner.label} routing preferences opened.`)}
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-[#e8edf4]/20 p-4">
            <div className="text-[11px] font-inter-b uppercase tracking-[0.16em] text-[#bdbdbd]">Status</div>
          ...
          <div className="rounded-2xl border border-[#e8edf4]/20 p-4">
            <div className="text-[11px] font-inter-b uppercase tracking-[0.16em] text-[#bdbdbd]">Latency</div>
            <div className="mt-2 font-inter-b text-white">{partner.latency}</div>
          </div>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between gap-4 rounded-2xl border border-[#e8edf4]/20 p-4 shadow-sm">
          <div>
            ...
            <div className="mt-2 text-sm font-inter-s text-white">{partner.lastSync}</div>
          </div>
          <button
            type="button"
            onClick={() => notify(`${partner.label} sync refreshed.`, "success")}
        ...
        <button
          type="button"
          onClick={() => notify(`${partner.label} logistics settings submitted.`, "success")}
