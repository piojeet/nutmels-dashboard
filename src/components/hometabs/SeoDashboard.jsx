...
const metrics = [
  { label: 'Average rank', value: '12.4', note: '+2.1 vs last month', icon: FiTarget, tone: 'teal' },
  { label: 'Search impressions', value: '842.5K', note: '+14.2% organic reach', icon: FiTrendingUp, tone: 'blue' },
  { label: 'Sitemap status', value: 'Healthy', note: 'Indexed', icon: TbSitemap, tone: 'green' },
  { label: 'AI entity trust %', value: '94%', note: 'Entity confidence', icon: FiShield, tone: 'violet', progress: 94 },
];
...
const keywordRows = [
  { keyword: 'saas analytics dashboard', tag: 'Target', position: 3, move: '+2', volume: '12,500', kd: 68, trend: [20, 26, 22, 38, 30, 52, 60], tone: '#2bc6c4' },
  { keyword: 'glassmorphism ui kit', tag: '', position: 1, move: '-', volume: '8,200', kd: 42, trend: [18, 18, 32, 16, 34, 50, 50], tone: '#aeb8df' },
  { keyword: 'data visualization tools', tag: '', position: 14, move: '-3', volume: '45,000', kd: 89, trend: [62, 55, 58, 40, 33, 24, 18], tone: '#ff7070' },
  { keyword: 'seo admin template', tag: '', position: 5, move: '+7', volume: '3,100', kd: 24, trend: [10, 14, 12, 22, 30, 35, 48], tone: '#2bc6c4' },
];

const initialErrors = [
  { path: '/old-pricing-page', hits: 142, seen: '2h ago', queued: false },
  { path: '/blog/seo-tips-2022', hits: 89, seen: '5h ago', queued: false },
  { path: '/features/v1-legacy', hits: 45, seen: '1d ago', queued: false },
  { path: '/promo/black-friday-23', hits: 12, seen: '2d ago', queued: false },
];

const initialAlerts = [
  { id: 'crawl', title: 'Crawl required', body: 'Metadata and nav structure changed', read: false },
  { id: 'links', title: 'Broken links found', body: '4 redirect actions recommended', read: false },
  { id: 'sitemap', title: 'Sitemap refreshed', body: 'Last sync finished 2 hours ago', read: true },
];
...
function ChartTooltip({ active, payload, label, palette }) {
  ...
  return (
    <div
      ...
      style={{ background: palette.panelStrong, borderColor: palette.border, color: palette.text }}
    >
...
function SeoDashboard() {
  ...
  const palette =
    themeName === 'dark'
      ? {
          shell: 'linear-gradient(180deg, rgba(255,255,255,0.9) 0%, rgba(243,245,251,0.96) 100%)',
          frame: 'rgba(129, 143, 184, 0.18)',
          panel: 'linear-gradient(180deg, rgba(255,255,255,0.9) 0%, rgba(247,248,252,0.92) 100%)',
      ...
      : {
          shell: 'linear-gradient(180deg, rgba(13,25,63,0.96) 0%, rgba(10,20,53,0.98) 100%)',
          frame: 'rgba(255,255,255,0.08)',
          panel: 'linear-gradient(180deg, rgba(18,31,72,0.94) 0%, rgba(15,27,63,0.97) 100%)',
  ...
  const visibleKeywords = useMemo(() => {
    return keywordRows.filter((item) => {
      const matchesQuery = item.keyword.toLowerCase().includes(query.trim().toLowerCase());
      const matchesFilter =
  ...
  const exportKeywords = () => {
    const csv = [['Keyword', 'Position', 'Change', 'Volume', 'KD'], ...visibleKeywords.map((item) => [item.keyword, item.position, item.move, item.volume, item.kd])];
  ...
  const queueRedirect = (path) => {
    setErrors((current) => current.map((item) => (item.path === path ? { ...item, queued: true } : item)));
  ...
  const accentMap = {
    teal: { iconBg: palette.tealGlow, iconColor: palette.teal, textColor: palette.teal },
    blue: { iconBg: palette.blueGlow, iconColor: palette.blue, textColor: palette.blue },
    green: { iconBg: palette.greenGlow, iconColor: palette.green, textColor: palette.green },
    violet: { iconBg: palette.violetGlow, iconColor: palette.violet, textColor: palette.violet },
  };
      ...
      <div className='relative z-10 grid gap-4 xl:grid-cols-[minmax(0,1fr)_350px]'>
        <div className='space-y-4'>
              ...
              <div className='flex flex-col gap-3 sm:flex-row'>
                ...
                <div className='relative'>
                  ...
                  >
                    <FiBell size={15} />
                    {unreadAlerts ? (
                      <span className='absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-[10px] font-inter-b text-[#071d2c]' style={{ background: '#FAAA21' }}>
                        {unreadAlerts}
                      </span>
                    ) : null}
                  </button>
                  ...
                  {openMenu === 'alerts' ? (
                    <MenuSurface palette={palette} className='w-[260px]'>
                      <div className='flex items-center justify-between px-2 pb-2'>
                        <div className='text-sm font-inter-b' style={{ color: palette.text }}>
                          Alerts
                        </div>
                        <button
                          type='button'
                      ...
                      <div className='space-y-1'>
                        {alerts.map((item) => (
                          <button
                            ...
                            type='button'
                            onClick={() => {
                              setAlerts((current) => current.map((alert) => (alert.id === item.id ? { ...alert, read: true } : alert)));
                              setOpenMenu('');
                            }}
                            className='block w-full cursor-pointer rounded-[14px] px-3 py-2 text-left transition'
                            style={{ background: item.read ? 'transparent' : palette.chip }}
                          >
                            <div className='flex items-start justify-between gap-3'>
                              <div>
                                <div className='text-sm font-inter-m' style={{ color: palette.text }}>
                                  {item.title}
                                </div>
                              ...
                              {!item.read ? <span className='mt-1 h-2.5 w-2.5 rounded-full bg-yellow-color' /> : null}
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <MenuSurface palette={palette}>
                      {periods.map((item) => (
                        <button
                          ...
                          style={{ background: period === item ? palette.chip : 'transparent', color: palette.text }}
                        >
                          {item}
                          {period === item ? <span className='text-yellow-color'>Active</span> : null}
                        </button>
                      ))}
                    </MenuSurface>
                  ) : null}
              ...
              {metrics.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className='flex items-start justify-between gap-3'>
                      <div>
                        <div className='text-[11px] uppercase tracking-[0.18em]' style={{ color: palette.muted }}>
                          {item.label}
                        </div>
                        <div className='mt-2 text-[33px] font-inter-b leading-none' style={{ color: palette.text }}>
                          {item.value}
                        </div>
                      </div>
                      <div className='flex h-8 w-8 items-center justify-center rounded-lg' style={{ background: accent.iconBg, color: accent.iconColor }}>
                        <Icon size={16} />
                      </div>
                    </div>
                    ...
                    {item.progress ? (
                      <div className='mt-5'>
                        <div className='h-1.5 overflow-hidden rounded-full' style={{ background: palette.progressTrack }}>
                          <div className='h-full rounded-full' style={{ width: `${item.progress}%`, background: `linear-gradient(90deg, ${palette.violet}, ${palette.teal})` }} />
                        </div>
                        <div className='mt-2 text-xs' style={{ color: accent.textColor }}>
                          {item.note}
                        </div>
                      </div>
                    ) : (
                      <div className='mt-5 text-xs' style={{ color: accent.textColor }}>
                        {item.note}
                      </div>
                    )}
                  </button>
          ...
          <div className='rounded-[24px] border p-4 sm:p-5' style={{ background: palette.panel, borderColor: palette.border }}>
                    ...
                    <MenuSurface palette={palette}>
                      {filters.map((item) => (
                        <button
                          ...
                          className='flex w-full cursor-pointer items-center justify-between rounded-[14px] px-3 py-2 text-sm transition'
                          style={{ background: filterLabel === item ? palette.chip : 'transparent', color: palette.text }}
                        >
                          {item}
                          {filterLabel === item ? <span className='text-yellow-color'>Active</span> : null}
                        </button>
                      ))}
                    </MenuSurface>
                  ) : null}
            ...
            <div className='mt-5 h-[250px] w-full sm:h-[290px]'>
              <ResponsiveContainer width='100%' height='100%'>
                <AreaChart data={chartSeries[period]} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                  <defs>
                    <linearGradient id='seoPerformanceFill' x1='0' y1='0' x2='0' y2='1'>
                      <stop offset='0%' stopColor={palette.teal} stopOpacity={palette.chartFillStart} />
                      <stop offset='100%' stopColor={palette.teal} stopOpacity={palette.chartFillEnd} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid vertical={false} stroke={palette.chartGrid} />
                  <XAxis dataKey='label' tickLine={false} axisLine={false} tick={{ fill: palette.muted, fontSize: 12 }} dy={8} />
                  <YAxis tickLine={false} axisLine={false} tick={{ fill: palette.muted, fontSize: 12 }} width={34} tickFormatter={(value) => `${Math.round(value / 1000)}k`} />
                  ...
                  <Area
                    ...
                    dot={false}
                    activeDot={{ r: 4, fill: palette.teal, stroke: palette.panelStrong, strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className='mt-4 overflow-x-auto'>
              <table className='min-w-full text-left text-sm'>
                <thead>
                  <tr className='border-t text-[11px] uppercase tracking-[0.18em]' style={{ borderColor: palette.border, color: palette.muted }}>
                    <th className='py-4 pr-4 font-inter-m'>Keyword</th>
                    <th className='py-4 pr-4 font-inter-m'>Position</th>
                    <th className='py-4 pr-4 font-inter-m'>Volume</th>
                    <th className='py-4 pr-4 font-inter-m'>KD %</th>
                    <th className='py-4 pr-4 font-inter-m'>Trend (7D)</th>
                    <th className='py-4 text-right font-inter-m'>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {visibleKeywords.length ? (
                    visibleKeywords.map((row) => {
                      const movementColor = row.move.startsWith('+') ? palette.green : row.move.startsWith('-') && row.move !== '-' ? palette.danger : palette.muted;
                      const kdColor = row.kd >= 75 ? palette.danger : row.kd >= 45 ? '#f6c24d' : palette.green;

                      return (
                        <tr key={row.keyword} className='border-t' style={{ borderColor: palette.border, color: palette.text }}>
                          <td className='py-4 pr-4'>
                            <div className='flex items-center gap-2'>
                              <span className='font-inter-m'>{row.keyword}</span>
                              {row.tag ? (
                                <span className='rounded-full px-2 py-1 text-[10px] font-inter-m uppercase tracking-[0.08em]' style={{ background: palette.tealGlow, color: palette.teal }}>
                                  {row.tag}
                                </span>
                              ) : null}
                            </div>
                          ...
                          <td className='py-4 pr-4'>
                            ...
                            <span style={{ color: movementColor }}>{row.move}</span>
                          </td>
                          ...
                          <td className='py-4 pr-4'>
                            <div className='flex items-center gap-3'>
                              <span className='font-inter-m' style={{ color: kdColor }}>
                                {row.kd}
                              </span>
                              <div className='h-1.5 w-16 overflow-hidden rounded-full' style={{ background: palette.progressTrack }}>
                                <div className='h-full rounded-full' style={{ width: `${row.kd}%`, background: kdColor }} />
                              </div>
                            </div>
                          </td>
                          <td className='py-4 pr-4'>
                          ...
                          <td className='py-4 text-right'>
                            <div className='relative inline-flex'>
                              <button
                                ...
                                style={{ background: palette.chip, borderColor: palette.border, color: palette.muted }}
                              >
                                <FiMoreVertical size={14} />
                              </button>

                              {openMenu === `row:${row.keyword}` ? (
                                <MenuSurface palette={palette} className='w-[170px] text-left'>
                                  <button
                                    type='button'
                                    onClick={() => handleKeywordAction('open', row)}
                                  ...
                                  <button
                                    type='button'
                                    onClick={() => handleKeywordAction('track', row)}
                                    className='flex w-full cursor-pointer items-center gap-2 rounded-[14px] px-3 py-2 text-sm transition'
                                    style={{ color: palette.text }}
                                  >
                                    <FiRefreshCw size={14} />
                                  ...
                                  <button
                                    type='button'
                                    onClick={() => handleKeywordAction('copy', row)}
                                    className='flex w-full cursor-pointer items-center gap-2 rounded-[14px] px-3 py-2 text-sm transition'
                                    style={{ color: palette.text }}
                                  >
                                    <FiCopy size={14} />
                    ...
                    <tr className='border-t' style={{ borderColor: palette.border }}>
                      <td colSpan='6' className='py-8 text-center text-sm' style={{ color: palette.muted }}>
        ...
        <div className='space-y-4 xl:pt-[86px]'>
            ...
            <div className='flex items-start justify-between gap-3'>
              <div>
                <div className='flex items-center gap-2 text-xl font-inter-b' style={{ color: palette.text }}>
                  <span className='flex h-8 w-8 items-center justify-center rounded-xl' style={{ background: palette.dangerGlow, color: palette.danger }}>
              ...
              <span className='rounded-full px-2 py-1 text-[10px] font-inter-m uppercase tracking-[0.14em]' style={{ background: '#ff6f7e', color: '#fff' }}>
            ...
            <div className='mt-5 space-y-3'>
              {errorItems.map((item) => (
                <div key={item.path} className='rounded-[18px] border p-3' style={{ background: palette.cardAccent, borderColor: palette.border }}>
                  ...
                  <div className='mt-1 flex items-center justify-between gap-3 text-xs' style={{ color: palette.muted }}>
          ...
          <div className='rounded-[24px] border p-4 sm:p-5' style={{ background: palette.panel, borderColor: palette.border }}>
            ...
            <div className='mt-5 text-2xl font-inter-b' style={{ color: palette.text }}>
              Crawl Required
            </div>
            <div className='mt-2 text-sm leading-6' style={{ color: palette.muted }}>
              Major structural changes detected. Request Google to re-crawl your site.
            </div>

            <div className='mt-5 rounded-[18px] border p-3 text-sm' style={{ background: palette.cardAccent, borderColor: palette.border, color: palette.muted }}>
            ...
            <button
              ...
              className='mt-5 flex w-full cursor-pointer items-center justify-center gap-2 rounded-[18px] px-4 py-3.5 text-sm font-inter-b transition hover:brightness-110'
              style={{ background: crawlRequested ? 'linear-gradient(135deg, #f6c24d, #ffd36a)' : `linear-gradient(135deg, ${palette.teal}, #41d5ca)`, color: palette.ctaText }}
            >
              <FiZap size={16} />
